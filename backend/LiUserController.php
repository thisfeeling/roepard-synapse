...
if ($query->rowCount() > 0) {
	...
	$output .= "<table class='table align-items-center mb-0  table-hover table-sm'>
		...
		<tr class='text-center '>
		  <th class='text-uppercase text-secondary text-xxs font-weight-bolder opacity-7'>Profile picture/Bio/Website</th>
	...
	while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
		...
		$output .= "<tbody class='table-dark'>
					...
					<td class='align-middle text-center text-sm'>
					  ...
					  <p class='text-xs text-secondary mb-0'>{$row['bio']}</p>
					  <p class='text-xs text-secondary mb-0'>{$row['website']}</p>
					</td>
					<td class='align-middle text-center text-sm'>
